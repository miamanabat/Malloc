...
double  internal_fragmentation() {
    ...
    // If heap size is zero, internal fragmentation will automatically return 0
    ...
    double fragmentation = (internal_frags) / Counters[HEAP_SIZE] * 100.0;
...
double  external_fragmentation() {
    ...
    Block *largest_free = FreeList.next;
    if (!largest_free) return 0;
    double all_free_memory = 0;

    // Loop through to add to the total free memory for computation
    for (Block* curr = FreeList.next; curr != &FreeList; curr = curr->next) { 
         if (curr->capacity > largest_free->capacity) {
             largest_free = curr;
         }
         all_free_memory += curr->capacity;
    }

    if (all_free_memory == 0) return 0; 
    double fragmentation = ((1 - (largest_free->capacity / all_free_memory)) * 100.0);
