...
Block * free_list_search_bf(size_t size) {
    ...
    Block *temp = NULL;
    for(Block *curr = FreeList.next; curr != &FreeList; curr = curr->next) {
        double difference = curr->capacity - size;
        if(curr->capacity >= size && difference < min) {
            temp = curr;
            min = difference;
        }
    }
...
Block * free_list_search_wf(size_t size) {
    ...
    for(Block *curr = FreeList.next; curr != &FreeList; curr = curr->next) {
        if (!temp) {
            ...
            if (curr->capacity  > temp->capacity) temp = curr;
        } 
    }
    return temp;
}
...
size_t  free_list_length() {
    ...
    // Loop through list and add up number of blocks
    for(Block *curr = FreeList.next; curr != &FreeList; curr = curr->next) {
        counter++;
    }
    return counter;
